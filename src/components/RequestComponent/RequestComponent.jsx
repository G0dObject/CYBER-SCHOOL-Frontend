...
const RequestComponent = () => {
	...
	const forms = [{ element: <Form1 /> }, { element: <Form2 /> }, { element: <Form3 /> }];
	return (
					...
					<div className="request__buttons">
						{activeTab !== 0 ? (
							<button
								...
								className="request__button request__button-back">
								назад
							</button>
						) : (
						...
						<button
							...
							className="request__button request__button-next">
							{activeTab !== 2 ? 'вперёд' : 'отправить'}
						</button>
					</div>
				</div>
			</Container>
		...
		<ul className="request-tabs">
			{tabs.map((tab, index) => (
				<li
					...
					className={`request-tab ${index === state.activeTab ? 'active' : 'inactive'}`}
					key={index}>
					{tab.text}
				</li>
			))}
...
const Form1 = () => {
	return (
		<Form
			illustration={<Illustration1 className="request-form__illustration" />}
			className="form1"
			title="Личные данные">
			<div className="request-form__col">
				<label className="request-form__item">
					<div className="request-form__label required">Фамилия</div>
					<input ref={lastnameref} type="text" className="request-form__input" />
				</label>
				<label className="request-form__item">
					<div className="request-form__label required">Имя</div>
					<input ref={firstnameref} type="text" className="request-form__input" />
				</label>
				<label className="request-form__item">
					<div className="request-form__label required">Отчество</div>
					<input ref={surnameref} type="text" className="request-form__input" />
				</label>
				<label className="request-form__item">
					<div className="request-form__label required">Образование</div>
					<select ref={educationref} className="request-form__select">
			...
			<div className="request-form__col">
				<label className="request-form__item">
					...
					<input ref={dayofbirthref} type="date" className="request-form__input" />
				</label>
				<label className="request-form__item">
					<div className="request-form__label required">Место рождения</div>
					<input ref={cityref} type="text" className="request-form__input" />
				</label>
				<label className="request-form__item">
					<div className="request-form__label required">Адрес регистрации</div>
					<input ref={adressref} type="text" className="request-form__input" />
				</label>
				<label className="request-form__item">
					<div className="request-form__label required">Семейное положение</div>
					<select ref={disiredregionref} className="request-form__select">
...
const Form2 = () => {
	return (
		<Form
			illustration={<Illustration2 className="request-form__illustration" />}
			className="form2"
	...
	return (
		<Form className={`form3 ${purpose}`} title="Направление обращения">
			{purpose === 'internship' || purpose === 'work' ? (
				<div className="request-form__col request-form__col_text-content">
					<div className="request-form__text-content">
						<p>
							<span>Уважаемые кандидаты!</span> В банке “Центр-инвест” разработан и
							успешно внедрен единый прорядок трудоустройства, предполагающий
							прохождение стажировки в банке по результатам успешного обучения в
							Центре финансовой грамотности населения.
						</p>
						<p>
							Бесплатный обучающий курс по банковскому делу в Центре финансовой
							грамотности направлен на формирование у студентов и выпускников вузов и
							сузов практических навыков, необходимых для успешной работы в банке.
							Срок обучения состяавляет от 1 месяца.
						</p>
						<p>
							Обучение предполагает проведение различных форм контроля знаний
							(тестирование, бизнес-игры, сдача методических пособий, собеседование и
							т.д.)
						</p>
						<p>
							Лучшие выпускники Центра финансовой грамотности получают приглашение на
							прохождение дальнейшей стажировки в банке
						</p>
					</div>
				</div>
			) : null}
			...
			<div className="request-form__col">
				...
				<div className="request-form__item">
					<div className="request-form__label required">
						Направление{' '}
						{purpose === 'practice'
							? 'практики'
							: purpose === 'internship'
...
const Form = ({ className, title, children, illustration }) => {
	...
	return (
		<form action="#" onSubmit={sendForm} className={`request-form ${className}`}>
			...
			<div className="request-form__body">{children}</div>
